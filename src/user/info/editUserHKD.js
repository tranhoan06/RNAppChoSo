...
import TTHKD from "./TTHKD";

const typeOptions = [
  { label: "Thông tin tài khoản", value: "Thông tin tài khoản" },
  { label: "Thông tin hộ kinh doanh", value: "Thông tin hộ kinh doanh" },
];

const editUserHKD = ({ navigation }) => {
  const replaceNav = () => {
    navigation.goBack();
  };

  const onPressHandler = (value) => {
    if (value === "Thông tin hộ kinh doanh") {
      setInitialShow(1);
    }
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.center}>
        ...
        <View style={{ width: "94%" }}>
          {!initialShow ? <OrdinaryPeople /> : <TTHKD />}
        </View>
      </View>
    </ScrollView>
  );
